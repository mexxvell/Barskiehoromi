...
def start(message):
    ...
    bot.send_message(message.chat.id, "👋 Добро пожаловать!", reply_markup=kb)
...
def team_menu(message):
    ...
    bot.send_message(message.chat.id, "👥 Команда ScanDream:", reply_markup=kb)

@bot.message_handler(func=lambda m: m.text == "🏷 О бренде")
def about_brand(message):
    bot.send_message(message.chat.id, "ScanDream — проект, объединяющий путешествия, йогу и стиль жизни.")

@bot.message_handler(func=lambda m: m.text == "🌐 Официальные источники")
def official_sources(message):
    bot.send_message(message.chat.id, "Instagram: https://instagram.com/scandream\nYouTube: https://youtube.com/@scandream")
