...
def about_brand(message):
    kb = types.ReplyKeyboardMarkup(resize_keyboard=True)
    kb.add("🌐 Официальные источники", "🔙 Назад к меню")
...
Проект йога-кемп - это творческая интеграция опыта и пользы. Пользы через новые знания и умения. Умения через новые формы.""", reply_markup=kb)
